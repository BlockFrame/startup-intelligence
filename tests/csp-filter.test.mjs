...
describe('CSP violation filter (shouldSuppressCspViolation)', () => {
  ...
  describe('connect-src HTTPS suppression (policy-aware)', () => {
    it('suppresses HTTPS connect-src when CSP allows https:', () => {
      assert.ok(suppress('enforce', 'connect-src', 'https://api.startupintelligence.app/api/oref-alerts', '', true));
    });
    ...
    it('does NOT suppress HTTPS connect-src when CSP does not allow https:', () => {
      assert.ok(!suppress('enforce', 'connect-src', 'https://api.startupintelligence.app/api/oref-alerts', '', false));
    });
  ...
  describe('scheme-only and special values', () => {
    ...
    it('suppresses blob: URI', () => {
      assert.ok(suppress('enforce', 'worker-src', 'blob:https://startupintelligence.app/abc', '', false));
    });
  ...
  describe('third-party noise', () => {
    ...
    it('suppresses manifest.webmanifest', () => {
      assert.ok(suppress('enforce', 'default-src', 'https://startupintelligence.app/manifest.webmanifest', '', false));
    });
  });
