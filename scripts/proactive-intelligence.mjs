...
async function upstashRest(...args) {
  const res = await fetch(`${UPSTASH_URL}/${args.map(encodeURIComponent).join('/')}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${UPSTASH_TOKEN}`, 'User-Agent': 'startupintelligence-proactive/1.0' },
...
async function isUserPro(userId) {
  ...
  try {
    const res = await fetch(`${CONVEX_SITE_URL}/relay/entitlement`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${RELAY_SECRET}`, 'User-Agent': 'startupintelligence-proactive/1.0' },
...
async function sendSlack(userId, webhookEnvelope, text) {
  try {
    ...
    const resp = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'User-Agent': 'startupintelligence-proactive/1.0' },
...
async function main() {
  ...
  for (const rule of rules) {
    ...
    const systemPrompt = `You are StartupIntelligence's proactive intelligence agent. Today is ${dateStr}.
...
Current snapshot: ${JSON.stringify({ topRiskCountries: currentLandscape.topRiskCountries, gpsZoneCount: currentLandscape.gpsZoneCount, unrestCount: currentLandscape.unrestCount })}`;
    ...
    const text = `PROACTIVE INTELLIGENCE BRIEF\n\n${brief}\n\n${'─'.repeat(40)}\nGenerated by StartupIntelligence AI · startupintelligence.app`;
    const subject = `StartupIntelligence Intelligence Update — ${dateStr}`;
