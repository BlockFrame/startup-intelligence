 ...
 *
 * Injects a Clerk Bearer token (or STARTUP_INTELLIGENCE_API_KEY as fallback) directly
...
function reportServerError(res: Response, input: RequestInfo | URL): void {
  ...
  try {
    ...
    const path = new URL(href, globalThis.location?.href ?? 'https://startupintelligence.app').pathname;
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-Startup-Intelligence-Key')) {
    const res = await globalThis.fetch(input, init);
    reportServerError(res, input);
    return res;
  }

  // 1. STARTUP_INTELLIGENCE_API_KEY from env (desktop / test environments).
  try {
    ...
    if (startupIntelligenceKey) {
      existing.set('X-Startup-Intelligence-Key', startupIntelligenceKey);
  ...
  // Try si-pro-key first, then si-widget-key. A relay-only pro key can be invalid
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-Startup-Intelligence-Key', testerKey);
  ...
  //    none of the tester keys are in STARTUP_INTELLIGENCE_VALID_KEYS).
  try {
