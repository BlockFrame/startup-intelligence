...
const DEFAULT_WEB_API_URL = 'https://api.startupintelligence.app';
...
function isStartupIntelligenceWebHost(hostname: string): boolean {
  return hostname === 'startupintelligence.app'
    || hostname === 'startupintelligence.app'
    || hostname.endsWith('.startupintelligence.app');
}
...
export function getConfiguredWebApiBaseUrl(): string {
  ...
  if (!isStartupIntelligenceWebHost(hostname)) {
    return '';
  }
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://startupintelligence.app';
  return '';
}
...
const APP_HOSTS = new Set([
  ...
  'startupintelligence.app',
  'tech.startupintelligence.app',
  'api.startupintelligence.app',
  'localhost',
  '127.0.0.1',
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.startupintelligence.app');
  } catch {
    return false;
  }
}
...
export function installRuntimeFetchPatch(): void {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const debug = localStorage.getItem('si-debug-log') === '1';
    ...
    if (allowCloudFallback && !isKeyFreeApiTarget(target)) {
      try {
        ...
        const wmKeyState = getSecretState('STARTUP_INTELLIGENCE_API_KEY');
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        const wmKeyValue = getRuntimeConfigSnapshot().secrets['STARTUP_INTELLIGENCE_API_KEY']?.value;
        if (wmKeyValue) {
          cloudHeaders.set('X-Startup-Intelligence-Key', wmKeyValue);
        }
      }
...
const ALLOWED_REDIRECT_HOSTS = /^https:\/\/([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*startup_intelligence\.app(:\d+)?$/;
...
export function installWebApiRedirect(): void {
   ...
   *   2. STARTUP_INTELLIGENCE_API_KEY from runtime config → X-Startup-Intelligence-Key
   *   3. Tester key (si-pro-key / si-widget-key) → X-Startup-Intelligence-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    if (headers.has('Authorization') || headers.has('X-Startup-Intelligence-Key')) return init;
    // STARTUP_INTELLIGENCE_API_KEY from env or runtime config
    try {
      const { getRuntimeConfigSnapshot } = await import('@/services/runtime-config');
      const startupIntelligenceKey = getRuntimeConfigSnapshot().secrets['STARTUP_INTELLIGENCE_API_KEY']?.value;
      if (startupIntelligenceKey) {
        headers.set('X-Startup-Intelligence-Key', startupIntelligenceKey);
        return { ...init, headers };
      }
    } catch { /* runtime-config unavailable — fall through */ }
    // Tester key (si-pro-key / si-widget-key): forward as API key header.
    ...
    if (testerKey) {
      headers.set('X-Startup-Intelligence-Key', testerKey);
      return { ...init, headers };
    }
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://api.startupintelligence.app/api/...) — just inject auth.
