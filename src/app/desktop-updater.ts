...
export class DesktopUpdater implements AppModule {
  ...
  private async checkForUpdate(): Promise<void> {
    try {
      const res = await fetch('https://api.startupintelligence.app/api/version', {
      ...
      const dismissKey = `si-update-dismissed-${remote}`;
      ...
      const releaseUrl = typeof data.url === 'string' && data.url
        ? data.url
        : 'https://github.com/koala73/startup-intelligence/releases/latest';
  ...
  private async resolveUpdateDownloadUrl(releaseUrl: string): Promise<string> {
    try {
      ...
      if (platform) {
        ...
        return `https://api.startupintelligence.app/api/download?platform=${platform}&variant=${variant}`;
      }
    } catch {
  ...
  private async showUpdateToast(version: string, releaseUrl: string): Promise<void> {
    ...
    const dismissToast = () => {
      setDismissed(`si-update-dismissed-${version}`);
