...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!ent || ent.features.tier < 1) {
    return new Response(JSON.stringify({ error: 'pro_required', message: 'Discord notifications are available on the Pro plan.', upgradeUrl: 'https://startupintelligence.app/pro' }), { status: 403, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
  }
