...
<title>Authorize &#x2014; StartupIntelligence MCP</title>
<style>
...
.si-logo{display:flex;align-items:center;gap:.5rem;margin-bottom:2rem;text-decoration:none}
.si-logo svg{color:#2d8a6e}
.si-logo-text{font-size:.75rem;color:#555;letter-spacing:.1em;text-transform:uppercase}
...
<body>
<a href="https://startupintelligence.app" class="si-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="si-logo-text">StartupIntelligence MCP</span></a>
<div class="card">
<div class="client-hd">
...
<hr>
<form id="cf" method="POST" action="https://api.startupintelligence.app/oauth/authorize">
<input type="hidden" name="_nonce" id="nn" value="${escapeHtml(nonce)}">
<input type="hidden" name="_js" id="jf" value="">
<label for="api_key">API Key</label>
<input type="password" id="api_key" name="api_key" placeholder="si_&#8230;" autocomplete="current-password" required>
<p class="hint">No key? <a href="https://startupintelligence.app/pro" target="_blank" rel="noopener">Get one at startupintelligence.app/pro &#x2192;</a></p>
<p class="error" id="ke"${errorMsg ? '' : ' style="display:none"'}>${errorMsg ? escapeHtml(errorMsg) : ''}</p>
<button type="submit" id="ab">Authorize</button>
</form>
</div>
<p class="footer"><a href="https://startupintelligence.app" target="_blank" rel="noopener">startupintelligence.app</a> &middot; <a href="https://startupintelligence.app/pro" target="_blank" rel="noopener">Get an API key &#x2192;</a></p>
...
export default async function handler(req) {
  ...
  if (method === 'POST') {
    ...
    if (origin && origin !== 'https://api.startupintelligence.app' && origin !== 'null') {
