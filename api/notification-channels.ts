  ...
  try {
    const res = await fetch(`${UPSTASH_URL}/lpush/wm:events:queue/${encodeURIComponent(msg)}`, {
      ...
      headers: {
        ...
        'User-Agent': 'startupintelligence-edge/1.0',
      },
...
async function publishFlushHeld(userId: string, variant: string): Promise<void> {
  ...
  try {
    await fetch(`${UPSTASH_URL}/lpush/wm:events:queue/${encodeURIComponent(msg)}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${UPSTASH_TOKEN}`, 'User-Agent': 'startupintelligence-edge/1.0' },
      signal: AbortSignal.timeout(5000),
    });
  } catch (err) {
  ...
  if (req.method === 'POST') {
    ...
    if (!isTesterKey && (!ent || ent.features.tier < 1)) {
      return json({
        ...
        upgradeUrl: 'https://startupintelligence.app/pro',
      }, 403, corsHeaders);
    }
