...
Agent entry point for StartupIntelligence. Read this first, then follow links for depth.
...
│   └── startup_intelligence/       # Domain handlers (mirrors proto service structure)
├── proto/                  # Protobuf definitions (sebuf framework)
│   ├── buf.yaml            # Buf configuration
│   └── startup_intelligence/       # Service definitions with HTTP annotations
...
- `server/startup_intelligence/<domain>/` has RPC handlers matching proto services
...
1. Define proto message in `proto/startup_intelligence/<domain>/`
2. Add RPC with `(sebuf.http.config)` annotation
3. Run `make generate`
4. Create handler in `server/startup_intelligence/<domain>/`
